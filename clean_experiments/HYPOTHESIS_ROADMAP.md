...
7. **H6**: cosmological flow (`omega(mu)`) to spectrum-level observables
...
| Experiment 15 (M, planned) | Cosmological scenario from realistic RG profile `omega(mu)` | `clean_experiments/experiment_M_cosmo_flow.py` | predicted tilt/amplitude proxies vs control baselines | stable prediction bands across seeds |
...
Move to Experiment 15 (cosmological flow) using the same artifact standard and robust protocol.

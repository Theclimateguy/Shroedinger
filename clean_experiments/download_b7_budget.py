#!/usr/bin/env python3
"""Phase-7 download: TCWV, evaporation, total precipitation (6-hourly)
for the 12 regions x W9-W10.
...
def run_job(region: str, window: str, out_dir: Path) -> str:
    target = out_dir / f"era5_budget_{region}__{window}.nc"
    if target.exists() and target.stat().st_size > 1_000_000:
        return f"SKIP {region}__{window}"
    year, months = WINDOWS[window]
    request = {
        "product_type": ["reanalysis"],
        "variable": ["total_column_water_vapour", "evaporation",
                     "total_precipitation"],
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("data/b7budget"))

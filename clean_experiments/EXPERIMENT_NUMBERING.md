...
| Experiment 16 | N (Navier-Stokes moisture budget) | `clean_experiments/experiment_N_navier_stokes_budget.py` | `clean_experiments/results/experiment_N_navier_stokes_budget` | `clean_experiments/results/experiment_N_followup` |
| Experiment 17 | O (thermodynamic equilibrium + anomalies) | `clean_experiments/experiment_O_entropy_equilibrium.py` | `clean_experiments/results/experiment_O_entropy_equilibrium` | `clean_experiments/results/experiment_O_spatial_variance` |
...
- Experiment 16 follow-up variants are implemented in `clean_experiments/experiment_N_followup_dual.py` and stored under `clean_experiments/results/experiment_N_followup`.
- Experiment 17 includes spatial diagnostics in
  `clean_experiments/experiment_O_spatial_variance.py`,
  `clean_experiments/experiment_O_lambda_spatial_viz.py`,
  and `clean_experiments/experiment_O_spatial_active_west.py`.
...
- `clean_experiments/experiment_M_lambda_falsification_tests.py` is maintained as
  lettered sub-experiment **M4** (stages `S1/S2/S3`) inside the M-block and is not
  assigned a separate continuous ID.

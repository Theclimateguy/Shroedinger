...
\subsection{M4 staged falsification checks (S1/S2/S3)}
Additional artifact-control tests were run in \texttt{experiment\_M\_lambda\_falsification\_tests}:
\begin{itemize}
  \item S1 (placebo-\(\mu\)): \(\Delta_{\mathrm{OOF}}(\Lambda_{\mathrm{real}})=0.003412\),
    \(\Delta_{\mathrm{OOF}}(\Lambda_{\mathrm{placebo\text{-}\mu}})=-0.002986\),
    \(p_{\mathrm{perm}}^{\mathrm{placebo}}=1.0\).
  \item S2 (\(F^{\mathrm{comm}}\)-control): \(\Delta_{\mathrm{OOF}}(\Lambda_{\mathrm{comm}})=0\),
    \(\beta_{\mathrm{comm}}=0\).
  \item S3 (information criteria): \(\Delta\mathrm{AIC}=-13.202\), \(\Delta\mathrm{BIC}=-6.817\)
    for \(\mathrm{ctrl}\to\mathrm{ctrl}+\Lambda_{\mathrm{real}}\).
\end{itemize}

\subsection{Land/ocean closure split and detectability limit}
In \texttt{experiment\_M\_land\_ocean\_split}, with the same stable M setup:
\begin{itemize}
  \item ocean: gain \(=0.001818\), permutation \(p=0.021277\);
  \item land: gain \(=-0.000491\), permutation \(p=0.879433\).
\end{itemize}

Noise-probe diagnostics (\texttt{experiment\_M\_land\_ocean\_noise\_probe}) indicate:
\begin{itemize}
  \item removing \(P-E\) from target shifts land result from negative to near-zero;
  \item mild smoothing of full residual yields positive land gain (\(0.001357\), borderline \(p=0.0567\));
  \item ocean remains positive across all tested target variants.
\end{itemize}
Interpretation: over-land detectability in this closure metric is limited by target noise/parameterization, not by absence of Lambda-related physics.

\subsection{Spatial figure package for article}
\texttt{experiment\_M\_land\_ocean\_spatial\_viz} provides map-ready artifacts:
\begin{itemize}
  \item local \(R^2\)-gain map (\(R^2_{\mathrm{full}}-R^2_{\mathrm{base}}\), train\(\le\)2018, test=2019),
  \item local \(\beta_\Lambda\) map,
  \item land/ocean gain distributions.
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{clean\_experiments/results/experiment\_M\_lambda\_falsification\_tests/}
  \item \texttt{clean\_experiments/results/experiment\_M\_land\_ocean\_split/}
  \item \texttt{clean\_experiments/results/experiment\_M\_land\_ocean\_spatial\_viz/}
  \item \texttt{clean\_experiments/results/experiment\_M\_land\_ocean\_noise\_probe/}
\end{itemize}

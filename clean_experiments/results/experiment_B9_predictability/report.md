...
## Positive control
- error grows (12 h -> 168 h) in 94 % of region-windows, spread in 96 %
- pooled curves strictly monotone: r True, q True
- bands already saturated at the 12 h lead: 12 of 144 (per band 200-400 / 400-800 / 800-1600 km: [10, 2, 0])
- pass = True
...
## H9e: out-of-sample (2021-2022, 8 regions)

| statistic | Spearman rho | p (perm) | n |
|---|---|---|---|
| predicted vs observed lambda | +0.333 | 0.220 | 8 |
| A vs lambda (refitted in sample) | -0.333 | 0.789 | 8 |
| A vs mu | +0.262 | 0.262 | 8 |

out-of-sample R^2 vs training mean: 0.30133255295026007


## Jackknife (leave one region out, n = 12)

| pair | rho (full) | rho range | sign stable | most influential | rho without it |
|---|---|---|---|---|---|
| A_vs_lambda | -0.252 | -0.473 .. -0.073 | True | R11_EURO | -0.073 |
| A_vs_mu | +0.601 | +0.482 .. +0.845 | True | R10_INDO | +0.482 |
| A_vs_T50 | -0.510 | -0.773 .. -0.373 | True | R11_EURO | -0.373 |

## Exploratory (post-hoc, not a criterion)

_generated after the preregistered battery was scored and seen; not a criterion, reported so the pattern behind the null is visible_

| statistic | Spearman rho | p (perm, two-sided) |
|---|---|---|
| A_vs_initial_relative_error_r12h | +0.476 | 0.128 |
| A_vs_T50 | -0.510 | 0.093 |
| r_init_vs_lambda | -0.755 | 0.008 |
| A_vs_mu_after_removing_r_init_only | +0.559 | 0.077 |
| A_vs_lambda_after_removing_r_init_only | +0.077 | 0.796 |

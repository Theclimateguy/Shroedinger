...
def experiment_5_G2_pointerF(
    ...
    S_hor = np.array([k*DeltaS_hor for k in range(K_layers+1)], float)
    w_layers = np.exp(-S_hor[:-1]); w_layers = w_layers/np.sum(w_layers)
    ...
    def lambda_features(psis_local, layer_local):
        ...
        return {'Lambda_w':Lambda_w,'Lambda_unw':Lambda_unw,'Coh_ptr_w':Coh_ptr_w,'layer_mean':float(np.mean(layer_local))}
    ...
    for n in range(Nmu_steps):
        ...
        rows.append({'n':n,'omega_profile':omega_profile,'Delta_mu':Delta_mu,'K_layers':K_layers,
                     'dmu_mcwf':dmu_mcwf,'Gamma':Gamma,'p_jump':p_jump,'Ntraj':Ntraj,
                     'dS_hor_avg':dS_hor_avg,'dQ_in':dQ_in,'dQ_avg':dQ_avg,
                     'S_ens':S_ens,'dS_ens':dS_ens,
                     'jump_rate_emp':float(np.mean(layer!=layer_before)),
                     **feats})

    df=pd.DataFrame(rows)
    x=df['dQ_in'].values; y=df['dS_hor_avg'].values
    ...
    fit=pd.DataFrame([{'a_1_over_Teff':float(a),'b':float(b),'r2':float(r2),

...
# ==========================================================================
# 4. ПРОФИЛИ УГЛОВОЙ СКОРОСТИ omega(mu) — ИСПРАВЛЕННАЯ ВЕРСИЯ
# ==========================================================================

def experiment_4_omega_profiles(Delta_mu=2.0, K=64, r=0.4, beta=0.1,
                                alpha_state=np.pi/4):
    """
    Проверяем: Lambda_matter ∝ sin(∫ omega(μ) dμ) * <sigma_x>
    для трёх профилей omega(μ):

        1. Константа:   omega(μ) = omega_0
        2. Гауссов пакет: omega(μ) = omega_0 * exp(-(μ-μ_c)^2 / (2 σ^2))
        3. Осцилляции:  omega(μ) = omega_0 * cos(k_rg * μ)

    ВАЖНО: набег фазы и построение A(μ) используют ОДИН и тот же метод
    интегрирования — трапеций.
    """
    dmu    = Delta_mu / K
    mu_arr = np.linspace(0, Delta_mu, K + 1)
    rho0   = state_rho(alpha_state)
    ...
    for name, omega_func in profiles.items():
        ...
        # Накопленный угол методом трапеций на той же сетке
        cum_angle = np.zeros_like(mu_arr)
        # cum_angle[j] = ∫_{μ0}^{μ_j} omega(μ) dμ (трапеции, равномерная сетка)
        cum_angle[1:] = np.cumsum(0.5 * (omega_vals[:-1] + omega_vals[1:]) * dmu)

        # Полный угол
        phi_total = cum_angle[-1]   # совпадает с np.trapz(omega_vals, mu_arr)

        # Связности в узлах μ_j с тем же cum_angle
        A_list = [
            build_A(beta, r * np.cos(ca), r * np.sin(ca))
            for ca in cum_angle
        ]

        # Интеграл Lambda_matter по μ
        Lambda_sum = 0.0
        for k in range(K):
            F  = curvature_F(A_list[k], A_list[k+1], dmu)
            Fp = F_phys(F)
    ...
    df = pd.DataFrame(rows)
    df.to_csv('exp4_omega_profiles_fixed.csv', index=False)
    print("=== ЭКСПЕРИМЕНТ 4 (fixed): Профили omega(mu) ===")
    print(df.to_string(index=False, float_format=lambda x: f"{x:.16e}"))
    print("\nОжидаемо: rel_error_% ≲ 1e-14 для всех трёх профилей.")
    print("Сохранено: exp4_omega_profiles_fixed.csv\n")
    return df
